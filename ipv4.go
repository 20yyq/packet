...
// @ LastEditTime   : 2023-10-13 17:17:40
// @ LastEditors    : Eacher
// @ --------------------------------------------------------------------------------<
// @ Description  	:
// @ --------------------------------------------------------------------------------<
// @ FilePath       : /20yyq/packet/ipv4.go
// @@
package packet
...
var ipv4NativeEndian = binary.ByteOrder(binary.LittleEndian)

func init() {
	b := [4]byte{}
	*(*uint32)(unsafe.Pointer(&b[0])) = 1
	if b[0] != 1 {
		ipv4NativeEndian = binary.ByteOrder(binary.BigEndian)
	}
}
